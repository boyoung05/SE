...
본 시퀀스 다이어그램은 보호자 또는 아동이 서비스 이용을 위해 회원가입을 수행하는 흐름을 나타낸다.  
사용자가 회원가입 화면에서 아이디, 비밀번호, 이메일 등의 정보를 입력하면 시스템은 입력값의 누락 여부와 중복 여부를 확인한다.  
검증이 완료되면 사용자 계정 정보를 저장하고, 회원가입 완료 메시지를 출력한 후 로그인 화면으로 이동한다.

3.3.2 로그인을 한다  
![웰폰키즈 Sequence Diagram](../images/login.png)
본 시퀀스 다이어그램은 사용자가 서비스에 접속하기 위해 로그인을 수행하는 흐름을 나타낸다.  
사용자가 아이디와 비밀번호를 입력하면 시스템은 해당 정보가 등록된 사용자 정보와 일치하는지 확인한다.  
로그인이 성공하면 사용자 유형에 따라 보호자 또는 아동 메인 화면으로 이동하고, 실패 시 오류 메시지를 출력한다. 

3.3.3 아동 계정을 등록한다  
![웰폰키즈 Sequence Diagram](../images/kid-register.png) 
본 시퀀스 다이어그램은 보호자가 관리 대상 아동 계정을 등록하는 흐름을 나타낸다.  
보호자가 아동 정보와 기기 정보를 입력하면 시스템은 필수 입력값과 중복 등록 여부를 확인한다.  
검증이 완료되면 보호자 계정과 아동 계정을 연결하고, 등록 완료 메시지를 출력한다.

3.3.4 사용 리포트를 조회한다  
![웰폰키즈 Sequence Diagram](../images/report.png) 
본 시퀀스 다이어그램은 보호자가 아동의 스마트폰 사용 리포트를 조회하는 흐름을 나타낸다.  
보호자가 조회할 아동을 선택하면 시스템은 해당 아동의 사용 기록과 앱별 사용 정보를 조회한다.  
이후 전체 사용 시간, 앱별 사용 시간, 사용 추이를 분석하여 그래프와 요약 정보 형태로 제공한다.

3.3.5 사용 제한을 설정한다  
![웰폰키즈 Sequence Diagram](../images/limit-register.png)
본 시퀀스 다이어그램은 보호자가 아동의 스마트폰 또는 특정 앱 사용 제한을 설정하는 흐름을 나타낸다.  
보호자가 제한 대상 아동과 제한 조건을 입력하면 시스템은 입력값의 형식과 유효성을 확인한다.  
유효한 설정인 경우 사용 제한 정보를 저장하고, 설정 완료 메시지를 출력한다. 

3.3.6 미션을 설정한다  
![웰폰키즈 Sequence Diagram](../images/mission-register.png) 
본 시퀀스 다이어그램은 보호자가 아동에게 스마트폰 사용 습관 개선 미션을 부여하는 흐름을 나타낸다.  
시스템은 기본 미션 목록과 AI 추천 미션을 제공하고, 보호자는 미션 내용, 목표, 수행 기간을 설정한다.  
미션이 저장되면 시스템은 아동에게 미션 알림을 전송하고, 미션 설정 완료 메시지를 출력한다.

3.3.7 미션을 수행한다  
![웰폰키즈 Sequence Diagram](../images/do-mission.png)
본 시퀀스 다이어그램은 아동이 부여받은 미션을 확인하고 수행 결과를 기록하는 흐름을 나타낸다.  
아동이 진행 중인 미션을 선택하고 완료 버튼을 누르면 시스템은 미션 수행 상태를 완료로 변경한다.  
이후 보호자에게 미션 완료 알림을 전송하고, 아동에게 완료 메시지를 출력한다. 

3.3.8 AI 코칭 결과를 조회한다  
![웰폰키즈 Sequence Diagram](../images/AI.png)
본 시퀀스 다이어그램은 사용자가 AI 코칭 결과를 조회하는 흐름을 나타낸다.  
시스템은 아동의 사용 기록을 기반으로 생성된 AI 코칭 결과와 추천 미션 정보를 조회한다.  
조회된 결과는 스마트폰 사용 습관 분석, 개선 방향, 추천 미션 형태로 보호자 또는 아동에게 제공된다. 

3.3.9 알림을 확인한다  
![웰폰키즈 Sequence Diagram](../images/alarm.png)
본 시퀀스 다이어그램은 보호자 또는 아동이 시스템 알림을 확인하는 흐름을 나타낸다.  
사용자가 알림 화면에 접속하면 시스템은 전달된 알림 목록을 조회하여 제목, 내용, 발생 시간을 출력한다.  
사용자가 특정 알림을 선택하면 상세 내용을 보여주고, 해당 알림을 읽음 상태로 변경한다. 
